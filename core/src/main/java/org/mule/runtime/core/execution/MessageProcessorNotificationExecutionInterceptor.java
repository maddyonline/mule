...
public class MessageProcessorNotificationExecutionInterceptor implements MessageProcessorExecutionInterceptor {
  ...
  public MuleEvent execute(final MessageProcessor messageProcessor, final MuleEvent event) throws MessagingException {
    ...
    try {
      ...
      if (fireNotification) {
        fireNotification(event.getMuleContext().getNotificationManager(), event.getFlowConstruct(),
                         result != null ? result : event,
                         messageProcessor,
  ...
  public static void fireNotification(ServerNotificationManager serverNotificationManager, FlowConstruct flowConstruct,
                                      MuleEvent event, MessageProcessor processor, MessagingException exceptionThrown,
                                      int action) {
