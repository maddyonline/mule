...
/**
 * Abstract implementation of {@link MessageProcessor} that determines if processing should be performed blocking or
 * non-blocking..
 */
public abstract class AbstractNonBlockingMessageProcessor extends AbstractAnnotatedObject
    implements MessageProcessor, MessagingExceptionHandlerAware {
  ...
  public Publisher<MuleEvent> apply(Publisher<MuleEvent> publisher) {
    return from(publisher).concatMap(event -> create(emmiter -> {
      try {
        processNonBlocking(event, new CompletionHandler<MuleEvent, Throwable, MuleEvent>() {
